...
//   const verificationLink = `${process.env.VITE_API_BASE_URL}/verify-email/${token}`; // VITE_API_BASE_URL from your .env
//   const mailOptions = {
...
//     const resetLink = `${process.env.VITE_API_BASE_URL}/reset-password/${resetToken}`;
...
// Helper function to send verification email
const sendVerificationEmail = async (email, token) => {
  // VITE_API_BASE_URL for the backend API route that *receives* the click
...
//       const redirectUrl = `${process.env.FRONTEND_URL}/login?status=verification_failed&reason=invalid`;
//       return res.redirect(StatusCodes.SEE_OTHER, redirectUrl); // 303 See Other is good for redirects after POST/GET
//     }
...
//     if (user.is_verified) {
//       console.log("VerifyEmail: Email already verified for user:", user.userid);
//       // **REDIRECT ON ALREADY VERIFIED**
//       const redirectUrl = `${process.env.FRONTEND_URL}/login?status=already_verified`;
//       return res.redirect(StatusCodes.SEE_OTHER, redirectUrl);
//     }
...
//       const redirectUrl = `${process.env.FRONTEND_URL}/login?status=verification_failed&reason=expired`;
//       return res.redirect(StatusCodes.SEE_OTHER, redirectUrl);
//     }
...
//         "VerifyEmail: Failed to update user verification status. No rows affected."
//       );
//       // **REDIRECT ON INTERNAL SERVER ERROR DURING UPDATE**
//       const redirectUrl = `${process.env.FRONTEND_URL}/login?status=verification_error&reason=db_update`;
//       // Using 303 (See Other) is generally appropriate for redirects even in error cases,
//       // as it tells the client to perform a GET request to the new URL.
//       // A 500 status would typically be for an API response that *doesn't* redirect.
//       return res.redirect(StatusCodes.SEE_OTHER, redirectUrl);
//     }
...
//     );
//     // **REDIRECT ON SUCCESS**
//     // Redirect to a dedicated frontend success page
//     const redirectUrl = `${process.env.FRONTEND_URL}/email-verified-success`;
//     console.log(`VerifyEmail: Redirecting to frontend URL: ${redirectUrl}`);
//     res.redirect(StatusCodes.SEE_OTHER, redirectUrl); // 303 See Other is commonly used here

//   } catch (error) {
//     console.error("Error verifying email:", error);
//     // **REDIRECT ON GENERAL CATCH-ALL ERROR**
//     const redirectUrl = `${process.env.FRONTEND_URL}/login?status=verification_error&reason=server_error`;
//     res.redirect(StatusCodes.SEE_OTHER, redirectUrl); // Use 303 for redirect
//   }
// }
...
async function verifyEmail(req, res) {
    ...
    try {
        ...
        if (users.length === 0) {
            ...
            // Frontend will receive this JSON and handle navigation
            return res
                .status(StatusCodes.BAD_REQUEST)
                .json({ Msg: "Invalid or expired verification link." });
        }
