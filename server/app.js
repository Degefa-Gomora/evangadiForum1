...
// console.log(
//   "DEBUG: process.env.BASE_URL after dotenv.config():",
...
// console.log("--- ENV VARS LOAD CHECK ---");
// console.log("Backend process.env.BASE_URL:", process.env.BASE_URL);
// console.log("Backend process.env.EMAIL_USER:", process.env.EMAIL_USER); // Check another variable to see if dotenv is working at all
// console.log("--- END ENV VARS LOAD CHECK ---");

// const app = express();
// // Create HTTP server from the Express app BEFORE initializing Socket.IO
// const server = http.createServer(app);

// // --- CORS Configuration for HTTP Requests (Express) ---
// // IMPORTANT: Only one app.use(cors()) for HTTP requests.
// // We are explicitly allowing ONLY your frontend origin for security.
// // If you need to debug and temporarily allow all, change origin to "*"
// app.use(
//   cors({
//     origin: ["https://evanforumfend.degefagomora.com", "http://localhost:5173"], // ADDED localhost:5173 for development
//     methods: ["GET", "POST", "PUT", "DELETE"], // Include all HTTP methods your frontend uses
//     allowedHeaders: ["Content-Type", "Authorization"], // Include headers your frontend sends (e.g., for JWT)
//   })
// );
...
// // The `server` variable must be defined before `new Server(server, ...)`
// const io = new Server(server, {
//   cors: {
//     origin: ["https://evanforumfend.degefagomora.com", "http://localhost:5173"], // ADDED localhost:5173 for development
//     methods: ["GET", "POST"], // These are for Socket.IO's handshake, usually GET/POST are sufficient
//     // credentials: true // Might be needed if you're sending cookies/auth headers with WebSocket
//   },
// });

// // =================================================================
// // Your existing routes and middleware definitions start here
// // =================================================================
...
// const JWT_SECRET = process.env.JWT_SECRET || "supersecretjwtkey"; // Make sure to use a strong secret from .env
...
//     // Periodically clean up inactive users and broadcast updates
//     setInterval(() => {
//       const now = Date.now();
//       let usersChanged = false;
//       for (const userid in activeUsers) {
//         if (now - activeUsers[userid].lastActivity > ACTIVITY_TIMEOUT_MS) {
//           console.log(
//             `User ${activeUsers[userid].username} (ID: ${userid}) marked as inactive.`
//           );
...
// // app.use(
// //   cors({
// //     origin: "https://evanforumfend.degefagomora.com",
// //     methods: ["GET", "POST", "PUT", "DELETE"], // Include all HTTP methods your frontend uses
// //     allowedHeaders: ["Content-Type", "Authorization"], // Include headers your frontend sends (e.g., for JWT)
// //   })
// // );
...
// // const io = new Server(server, {
// //   cors: {
// //     origin: "https://evanforumfend.degefagomora.com", // Socket.IO CORS should also match your frontend
// //     methods: ["GET", "POST"], // These are for Socket.IO's handshake, usually GET/POST are sufficient
// //     // credentials: true // Might be needed if you're sending cookies/auth headers with WebSocket
// //   },
// // });
...
// //           socket.emit("registeredUsers", users);
// //         } catch (error) {
// //           // IMPORTANT: Add the catch block for the try here
// //           console.error(
// //             "Error fetching registered users for socket event:",
// //             error
// //           );
// //           socket.emit("error", "Failed to fetch registered users.");
// //         }
// //       });
// //     });
...
// -------------------------------------------------------------
// IMPORTANT: dotenv.config() MUST BE THE VERY FIRST EXECUTABLE LINE
// This ensures environment variables are loaded before anything else
// tries to access them.
// Ensure your .env file is in the same directory as this app.js.
// -------------------------------------------------------------
require("dotenv").config({ path: path.resolve(__dirname, ".env") });

// -------------------------------------------------------------
// DEBUGGING LOGS: Verify environment variables are loaded
// These logs should appear immediately after server startup.
// -------------------------------------------------------------
console.log('--- ENV VARS LOAD CHECK ---');
console.log('Backend process.env.BASE_URL:', process.env.BASE_URL);
console.log('Backend process.env.EMAIL_USER:', process.env.EMAIL_USER);
console.log('Backend process.env.JWT_SECRET:', process.env.JWT_SECRET ? 'Loaded' : 'NOT LOADED');
console.log('--- ALL ENVIRONMENT VARIABLES (Filtered) ---');
for (const key in process.env) {
  // Only log keys you care about to avoid exposing all variables
  if (
    key.startsWith("BASE_") ||
    key.startsWith("JWT_") ||
    key.startsWith("EMAIL_") ||
...
// --- CORS Configuration for HTTP Requests (Express) ---
// IMPORTANT: Only one app.use(cors()) for HTTP requests.
// We are explicitly allowing ONLY your frontend origin(s) for security.
app.use(
  cors({
    origin: ["https://evanforumfend.degefagomora.com", "http://localhost:5173"], // ADDED localhost:5173 for development
    methods: ["GET", "POST", "PUT", "DELETE"], // Include all HTTP methods your frontend uses
...
// Directly use process.env.JWT_SECRET. The dotenv package will have loaded it by now.
  ...
  try {
    ...
    const formattedMessages = messages.map((msg) => {
      return {
        ...
        file_name: msg.file_name || null,
        file_type: msg.file_type || null,
        // Convert file_data Buffer to Base64 string for direct use in frontend
        file_url: msg.file_data ? `data:${msg.file_type};base64,${msg.file_data.toString('base64')}` : null,
        audio_data: msg.audio_data || null,
        audio_type: msg.audio_type || null,
        audio_duration: msg.audio_duration || null,
      };
  ...
  try {
    ...
    io.on("connection", (socket) => {
      ...
      socket.on("sendMessage", async (msg) => {
        ...
        try {
          ...
          const newMessage = {
            ...
            file_name: null,
            file_type: null,
            file_url: null, // No file for text messages
            audio_data: null, // Ensure these are null for text messages
            audio_type: null,
            audio_duration: null,
          };
      ...
      socket.on("fileMessage", async (msg) => {
        ...
        try {
          ...
          const newMessage = {
            ...
            file_name,
            file_type,
            file_url: file_data ? `data:${file_type};base64,${file_data.toString('base64')}` : null, // Add file_url
            audio_data: null, // Ensure null for file messages
            audio_type: null,
            audio_duration: null,
          };
      ...
      socket.on("voiceMessage", async (msg) => {
        ...
        try {
          ...
          const newMessage = {
            ...
            is_deleted: false,
            reactions: [],
            file_data: null, // Ensure null for voice messages
            file_name: null,
            file_type: null,
            file_url: null, // No file for voice messages
            audio_data, // Include audio data
            audio_type,
            audio_duration,
          };
      ...
      socket.on("editMessage", async (data) => {
        ...
        try {
          ...
          const updatedMessage = {
            ...
            file_name: updatedMsgRows[0].file_name || null,
            file_type: updatedMsgRows[0].file_type || null,
            file_url: updatedMsgRows[0].file_data ? `data:${updatedMsgRows[0].file_type};base64,${updatedMsgRows[0].file_data.toString('base64')}` : null,
            audio_data: updatedMsgRows[0].audio_data || null,
            audio_type: updatedMsgRows[0].audio_type || null,
            audio_duration: updatedMsgRows[0].audio_duration || null,
          };
      ...
      socket.on("reactToMessage", async (data) => {
        ...
        try {
          ...
          const updatedMessage = {
            ...
            file_name: message.file_name || null,
            file_type: message.file_type || null,
            file_url: message.file_data ? `data:${message.file_type};base64,${message.file_data.toString('base64')}` : null,
            audio_data: message.audio_data || null,
            audio_type: message.audio_type || null,
            audio_duration: message.audio_duration || null,
          };
      ...
      socket.on("typing", (data) => {
        ...
        // Ensure the sender's own socket doesn't get the 'typing' event
        socket.to(roomToSend).emit("typing", {
          userid: data.userid,
          username: data.username,
          roomId: roomToSend,
        ...
        // Update last activity for the typing user
        if (activeUsers[data.userid]) {
          activeUsers[data.userid].lastActivity = Date.now();
        }
      });

      // Handles user stop typing indicator
      socket.on("stopTyping", (data) => {
        // Broadcast to others in the room that someone has stopped typing, exclude sender
        const roomToSend =
          data.message_type === "private" && data.recipient_id
            ? getPrivateChatRoomId(data.userid, data.recipient_id)
            : data.roomId || PUBLIC_CHAT_ROOM_ID;
      ...
      socket.on("disconnect", () => {
        console.log(`Client disconnected: ${socket.id}`);
        // Remove user from active users if their socket ID matches
        for (const userid in activeUsers) {
          if (activeUsers[userid].sid === socket.id) {
            delete activeUsers[userid];
    ...
    setInterval(() => {
      ...
      for (const userid in activeUsers) {
        if (now - activeUsers[userid].lastActivity > ACTIVITY_TIMEOUT_MS) {
          console.log(
            `User ${activeUsers[userid].username} (ID: ${userid}) inactive, removing.`
          );
          delete activeUsers[userid];
          changed = true;
        }
      ...
      const currentActiveUsersCount = Object.keys(activeUsers).length;
      if (changed || currentActiveUsersCount !== lastKnownActiveUsersCount) {
        io.emit(
          "onlineUsers",
          Object.values(activeUsers).map((u) => ({
            userid: u.userid,
        ...
        lastKnownActiveUsersCount = currentActiveUsersCount;
        console.log(`Updated online users broadcasted. Count: ${lastKnownActiveUsersCount}`);
      }
    }, 30 * 1000); // Check every 30 seconds
    ...
    console.error("Failed to start server:", error);
    process.exit(1); // Exit process if server can't start due to DB or other issues
  }
}
